...
async def search_query(request: SearchRequest):
    ...
    answer = f"""[演示模式] 這是針對您的查詢 "{request.query}" 的模擬回應。
...
請設置 PostgreSQL 並配置您的 OpenAI API 金鑰，然後重啟完整版本。

目前演示功能：
✓ 文件上傳與存儲
✓ 文件列表與管理
✗ AI 文本提取 (需要 OpenAI API)
✗ 語義搜尋 (需要向量資料庫)
✗ RAG 查詢回答 (需要 OpenAI API)"""
    
    sources = [
        {
            "document_id": "demo-doc-1",
            "chunk_index": 0,
            "text": "[演示] 這是一個範例來源片段。真實來源將來自您上傳的文件。",
            "score": 0.95,
            "metadata": {"demo": True}
        }
    ]
